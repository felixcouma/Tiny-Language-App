...
- `src/screens/` — Setup ("how many children?"), ProfilePicker, Home, Learning, Today, Collection, Parent, SoundGame/TwinMode/Phonics (all via `ChoiceGame`), **Phrase** (Word Practice / Phrase Builder), **Grid** (Word Board AAC + "Find" word-focus mode), **Song** ("Sing with Pip" — a transport-bar player: shuffle · prev · play/pause · next; auto-advances the enabled-songs queue and stops at the end, no loop; `verify-song-player.mjs`), Chant, Rest.
